...
namespace sdds {
    ...
    {
        os << setfill('0') << setw(3) << right << m_idOfSta;
        os << " | " << setw(m_widthField) << setfill(' ') << left << m_nameOfSta;
        ...
        if (f)
        {
            ...
            os << left << m_descriptionOfSta << endl;
        }
        else os << endl;
    }
    Station::Station(string& record)
    {
        ...
        size_t ps = 0;
        bool other;
        m_idOfSta = id_generator++;

        try {

            m_nameOfSta = ut.extractToken(record, ps, other);
            ...
            m_descriptionOfSta = ut.extractToken(record, ps, other);
